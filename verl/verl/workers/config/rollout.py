...
class SamplingConfig(BaseConfig):
    ...
    do_sample: bool = True
    n: int = 1
    # When set, overrides ``rollout.response_length`` for this sampling call
    # (used via ``val_kwargs`` so validation can generate longer responses than
    # training without bumping training memory). ``None`` falls back to the
    # training-side value.
    response_length: Optional[int] = None
    # When set, overrides ``rollout.prompt_length`` for this sampling call.
    # Used via ``val_kwargs`` so validation can pad to longer prompts than
    # training (e.g. long-context benchmarks with 20k+ instruction chains)
    # without affecting training tensor sizes. ``None`` falls back to the
    # training-side value. Only affects ``_agent_loop_postprocess`` padding —
    # the rollout server itself reads the raw ``prompt_ids`` length.
