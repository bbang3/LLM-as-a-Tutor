...
class RewardLoopWorker:
    ...
    def _init_reward_fn(self):
        ...
        if reward_loop_source == "register":
            ...
            assert (
                reward_loop_module_path is not None
            ), "reward_loop_module_path must be set when reward_loop_source='importlib'"
            assert (
                reward_loop_class_name is not None
    ...
    async def _preprocess_reward_inputs(self, data: DataProto) -> str:
        ...
        # @bbang3: remove <think> </think> from the response
        enable_thinking = self.config.data.get("apply_chat_template_kwargs", {}).get("enable_thinking", False)
        if enable_thinking:
            if "<think>" in rollout_response:
                if "</think>" in rollout_response:
                    rollout_response = rollout_response.split("</think>")[-1].strip()
                else:
                    rollout_response = "<invalid_response>"
            else:
                logger.warning(f"Thinking is enabled, but <think> is not found in the response: {rollout_response}")
...
class RewardLoopManager:
    ...
    def __init__(self, config: DictConfig, rm_resource_pool: RayResourcePool = None):
        ...
        if rm_cfg.enable:
            # When ``data_generator.mode == "eva_baseline_colocated"`` and
            # ``scalar_rm_path`` is set, RewardModelManager hosts a second
            # vLLM engine on the same GPU pool as the judge and rotates the
            # two via wake/sleep with a name argument. Otherwise
            # ``scalar_rm_config`` is None and only the judge engine runs.
            scalar_rm_config = self._extract_scalar_rm_config(config)
            self.reward_model_manager = RewardModelManager(
                rm_cfg, rm_resource_pool, scalar_rm_config=scalar_rm_config
            )
    ...
    @staticmethod
    def _extract_scalar_rm_config(config: DictConfig) -> dict | None:
        """Pull the EVA-baseline scalar-RM block out of ``data_generator``.

        Returns ``None`` (no second engine) unless ``data_generator.mode ==
        "eva_baseline_colocated"`` AND ``scalar_rm_path`` is set, so a stray
        ``scalar_rm_path`` in a non-EVA YAML cannot spin up the scalar RM.
        """
        dg = config.get("data_generator", None)
        if dg is None:
            return None
        if dg.get("mode", None) != "eva_baseline_colocated":
            return None
        path = dg.get("scalar_rm_path", None)
        if not path:
            return None
        return {
            "path": path,
            "max_model_len": int(dg.get("scalar_rm_max_model_len", 4096)),
        }
