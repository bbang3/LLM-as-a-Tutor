...
class Tracking:
    ...
    def __init__(
        self, project_name, experiment_name, entity_name, default_backend: str | list[str] = "console", config=None
    ):
        ...
        if "tracking" in default_backend or "wandb" in default_backend:
            ...
            # Persist wandb run id under the checkpoint dir so that resuming
            # from the same dir continues the same wandb run instead of
            # creating a fresh one.
            run_id, resume = None, None
            ckpt_dir = (config or {}).get("trainer", {}).get("default_local_dir")
            if ckpt_dir:
