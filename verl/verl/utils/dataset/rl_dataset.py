...
class RLHFDataset(Dataset):
    ...
    @staticmethod
    def _align_schemas(dataframes: list["datasets.Dataset"]) -> list["datasets.Dataset"]:
        """Fill columns missing in some datasets with ``None`` so that
        ``datasets.concatenate_datasets`` does not raise a schema-mismatch error.

        This is needed when validating on multiple benchmarks that have
        benchmark-specific columns (e.g. IFBench's ``instruction_id_list`` /
        ``kwargs`` vs FollowBench's ``level`` / ``evolve_instructions``).
        Reward functions already route on ``data_source`` and only access the
        columns listed in ``keep_keys``, so ``None`` values in non-matching
        columns are safely ignored.
        """
        if len(dataframes) <= 1:
            return dataframes

        # Collect the union of all features, first-seen type wins for each name.
        union_features: dict[str, datasets.features.FeatureType] = {}
        for df in dataframes:
            for name, feat in df.features.items():
                if name not in union_features:
                    union_features[name] = feat

        aligned = []
        for df in dataframes:
            for col, feat in union_features.items():
                if col not in df.column_names:
                    df = df.add_column(name=col, column=[None] * len(df), feature=feat)
            aligned.append(df)
    ...
    def _read_files_and_tokenize(self):
        ...
        for parquet_file in self.data_files:
            ...
            if parquet_file.endswith(".parquet"):
                ...
                # HuggingFace Hub dataset ID (e.g. "username/dataset")
                dataframe = datasets.load_dataset(parquet_file)["train"]
            dataframes.append(dataframe)
        dataframes = self._align_schemas(dataframes)
