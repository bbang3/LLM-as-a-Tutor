...
class WorkerHelper:
    ...
    @staticmethod
    def _get_free_port():
        # Set VERL_MASTER_PORT_RANGE="<start>-<end>" (inclusive) to bias picks
        # away from other Ray clusters on the same host; otherwise OS-assigned.
        port_range = os.getenv("VERL_MASTER_PORT_RANGE")
        if port_range:
            import random

            start_str, end_str = port_range.split("-", 1)
            candidates = list(range(int(start_str), int(end_str) + 1))
            ...
            for port in candidates:
                try:
                    ...
                    continue
            raise RuntimeError(f"No free port in VERL_MASTER_PORT_RANGE={port_range}")
