...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    # Per-prompt score std: reshape (n_prompts, n_per_prompt) then std per row
    uids = batch.non_tensor_batch["uid"]
    n_prompts = len(set(uids))
    n_per_prompt = len(sequence_score) // n_prompts
    score_std_mean = sequence_score.reshape(n_prompts, n_per_prompt).std(dim=1).mean().item()

    reward_mean = torch.mean(non_aborted_sequence_reward).detach().item()
    reward_max = torch.max(non_aborted_sequence_reward).detach().item()
    reward_min = torch.min(non_aborted_sequence_reward).detach().item()
    reward_std_mean = sequence_reward.reshape(n_prompts, n_per_prompt).std(dim=1).mean().item()
    ...
    metrics = {
        ...
        "critic/score/std": score_std_mean,
        # reward
        "critic/rewards/mean": reward_mean,
        "critic/rewards/max": reward_max,
        "critic/rewards/min": reward_min,
        "critic/rewards/std": reward_std_mean,
        # adv
